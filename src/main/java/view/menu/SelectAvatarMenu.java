...
public class SelectAvatarMenu extends Application {
    ...
    private void selectRandomAvatar() {
        int randomNumber = (int) (Math.random() * 8) + 1;
        String avatarPath = "/images/Avatars/" + randomNumber + ".png";

        try {
            selectedAvatar.setImage(new Image(getClass().getResource(avatarPath).toString()));
            UserController.choosingAvatarFromTemplates(randomNumber);
        } catch (Exception e) {
            e.printStackTrace();
            GameViewController.alertShowing(Alert.AlertType.ERROR,
                    "Error", "Random Avatar Selection Failed",
...
//
//    private void selectRandomAvatar() {
//        int randomNumber = (int) (Math.random() * 8) + 1; // عدد تصادفی بین 1 تا 8
//        String avatarPath = "/images/Avatars/" + randomNumber + ".png";
//
//        try {
//            selectedAvatar.setImage(new Image(getClass().getResource(avatarPath).toString()));
